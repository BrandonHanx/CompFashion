...
from .utils import reshape_text_features_to_concat
...
class AttentionModule(nn.Module):
    ...
    def forward(self, x, t, return_map=False, *args, **kwargs):
        ...
        values = values.view(b * self.n_heads, self.c_per_head, h, w).view(
            b * self.n_heads, self.c_per_head, h * w
        )
        ...
        global_cross_att_map = global_cross_att_map.view(
            b, self.n_heads, 1, h * w
        )  # (b, num_heads, 1, h * w)
        att_map = self_att_map + global_cross_att_map  # (b, num_heads, h * w, h * w)
        att_map_reshaped = att_map.view(
            b * self.n_heads, h * w, h * w
        )  # (b * num_heads, h * w, h * w)

        att_out = torch.bmm(
            values, att_map_reshaped.transpose(1, 2)
...
class SelfAttentionMap(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        keys = keys.view(b * self.n_heads, self.c_per_head, h, w).view(
            b * self.n_heads, self.c_per_head, h * w
        )
        queries = queries.view(b * self.n_heads, self.c_per_head, h, w).view(
            b * self.n_heads, self.c_per_head, h * w
        )

        att_map = torch.bmm(queries.transpose(1, 2), keys) / (self.c_per_head ** 0.5)
        att_map = self.softmax(
            att_map
...
class GlobalCrossAttentionMap(nn.Module):
    ...
    def forward(self, x, t):
        ...
        att_map = torch.bmm(x_reshape.transpose(1, 2), t_mapped).squeeze(-1) / (
            self.c_per_head ** 0.5
        )
